...
async function setupProviderAndSigner(impersonatedAccount) {
  try {
    if (!process.env.MAINNET_RPC_URL) {
      throw new Error(
        "Missing environment variable: MAINNET_RPC_URL. Please set it in the .env file.",
      );
    }
    ...
    if (!network) {
      throw new Error(
        "Fork connection failure. Hardhat network is unreachable.",
      );
    }
    ...
    if (!signer || !signer.address) {
      throw new Error(
        "Failed to get valid impersonated signer or invalid whale impersonation.",
      );
    }

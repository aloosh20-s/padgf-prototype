...
async function getQuote(router, amountIn, path) {
  try {
    ...
    if (!amountsOut || amountsOut.length === 0)
      throw new Error("Empty quote returned from DEX.");
...
async function executeSwap(
  ...
  try {
    ...
    let baseMaxFee =
      feeData.maxFeePerGas || hre.ethers.parseUnits("50", "gwei");
    let basePriority =
      feeData.maxPriorityFeePerGas || hre.ethers.parseUnits("1.5", "gwei");

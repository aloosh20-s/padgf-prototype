...
function runTests() {
  ...
  assert.strictEqual(
    metrics.attacker_net_profit_usdc <= 0,
    true,
    "Net profit should be <= 0 with no slippage",
  );
  ...
  assert.strictEqual(
    metrics.attacker_net_profit_usdc < 0,
    true,
    "High gas makes this unprofitable",
  );
  assert.strictEqual(
    metrics.normalized_risk_score > 0,
    true,
    "Should have a non-zero risk score from slippage alone",
  );
  console.log(
    "✔ Test 2 Passed: High Gas makes moderate slippage unprofitable => Non-zero Risk from Slippage",
  );
  ...
  assert.strictEqual(
    metrics.attacker_net_profit_usdc > 70 &&
      metrics.attacker_net_profit_usdc < 80,
    true,
  );
  assert.strictEqual(metrics.normalized_risk_score > 0, true);
  console.log(
    "✔ Test 3 Passed: Large loss / Low Gas with defaults (0.3% fee, 90% bribe) => Net Profit: $" +
      metrics.attacker_net_profit_usdc.toFixed(2),
  );

  // Test 4: High builder bribe (99%) reduces attacker profitability
  metrics = calculateRisk("10.0", "30000", "29000", "30000000000", {
    builderBribePercentage: 0.99,
  });
  assert.strictEqual(
    metrics.attacker_net_profit_usdc < 0,
    true,
    "99% bribe makes this unprofitable",
  );
  console.log(
    "✔ Test 4 Passed: High builder bribe (99%) makes net profit negative ($" +
      metrics.attacker_net_profit_usdc.toFixed(2) +
      ")",
  );
  ...
  metrics = calculateRisk("10.0", "30000", "29000", "30000000000", {
    builderBribePercentage: 0.5,
  });
  assert.strictEqual(metrics.attacker_net_profit_usdc > 400, true);
  console.log(
    "✔ Test 5 Passed: Lower builder bribe (50%) increases attacker net profit ($" +
      metrics.attacker_net_profit_usdc.toFixed(2) +
      ")",
  );
  ...
  console.log(
    "✔ Test 6 Passed: Higher pool fee (1%) reduces gross profit to $" +
      metrics.attacker_gross_profit_usdc.toFixed(2),
  );
  ...
  console.log(
    "✔ Test 7 Passed: Victim output loss exists but net profit is negative ($" +
      metrics.attacker_net_profit_usdc.toFixed(2) +
      ")",
  );

...
async function runScenario(scenarioData) {
    const { name, targetRisk, jsonFilename, gasPriceGwei, maxSlippageTolerance } = scenarioData;
    ...
    for (let frontrunAmt of [0, 5, 10, 20, 50, 80, 120, 200, 300, 500]) {
        ...
        if (frontrunAmt > 0) {
            const aAmountIn = hre.ethers.parseUnits(frontrunAmt.toString(), WETH_DECIMALS);
        ...
        let matches = false;
        if (targetRisk === "Low" && decisionResult.decision === "Execute") matches = true;
        if (targetRisk === "Moderate" && decisionResult.decision === "Delay") matches = true;
        if (targetRisk === "High" && decisionResult.decision === "Block") matches = true;

        if (matches) {
            console.log(`Found matching params: frontrun=${frontrunAmt} WETH. Score=${riskMetrics.normalized_risk_score.toFixed(4)}, Decision=${decisionResult.decision}`);
            ...
            if (decisionResult.execution_allowed) {
                ...
                const receipt = await executeSwap(router, weth, amountIn, simulatedQuoteWei, swapPath, signer, maxSlippageTolerance);
            ...
            const outputDeviationStr = (Number(referenceOutput) - Number(actualOutput)).toFixed(6);
            ...
            bestParams = result;
            found = true;
            fs.writeFileSync(path.join(customDir, jsonFilename), JSON.stringify(result, null, 2));
            break;
        }
    }
    
    if (!found) {
        throw new Error(`Could not find parameters to satisfy ${targetRisk} risk criteria.`);
    }
...
async function main() {
    try {
        ...
        let csvContent = "scenario_name,risk_category_target,input_amount,slippage_tolerance,reference_output,simulated_output,actual_output,output_deviation,slippage_deviation,price_impact,normalized_risk_score,risk_level,padgf_decision,execution_allowed,execution_status,estimated_gas_cost_eth,estimated_gas_price_gwei,artifact_filename\n";
        ...
        for (const r of results) {
            ...
            if (r.simulated_output === r.actual_output) simActOut = `${Number(r.actual_output).toFixed(2)}`;
            console.log(`| ${r.scenario_name.padEnd(8)} | ${r.input_amount.padStart(12)} | ${r.slippage_tolerance.padStart(18)} | ${Number(r.reference_output).toFixed(2).padStart(16)} | ${simActOut.padStart(23)} | ${r.slippage_deviation.padStart(18)} | ${r.price_impact.padStart(12)} | ${r.normalized_risk_score.padStart(10)} | ${r.risk_level.padEnd(10)} | ${r.padgf_decision.padEnd(14)} | ${r.execution_status.padEnd(16)} |`);
        }
    } catch (err) {
        fs.appendFileSync(path.join(__dirname, '..', 'debug.log'), `MAIN ERROR: ${err.stack}\n`);
    }
}

main().catch(err => {
    fs.appendFileSync(path.join(__dirname, '..', 'debug.log'), `TOP LEVEL ERROR: ${err.stack}\n`);
});

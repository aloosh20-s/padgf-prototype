...
async function run() {
  console.log(
    `[Exploratory Mode] Stage: ${DEMO_STAGE} | Input: ${CUSTOM_AMOUNT} WETH | Slippage: ${CUSTOM_SLIPPAGE}%`,
  );
  ...
  if (volatilityVol > 0) {
    console.log(
      `[Exploratory Simulation] Generating controlled manipulated-pool-state condition: ${volatilityVol} WETH traded ahead...`,
    );
  ...
  let result = {
    ...
    attacker_gross_profit_usdc:
      riskMetrics.attacker_gross_profit_usdc.toFixed(6),
  ...
  if (DEMO_STAGE === "EXECUTE") {
    ...
    console.log(
      `[Exploratory Execution] Proceeding with ${DEMO_EXEC_TYPE} execution mode...`,
    );
    ...
    if (DEMO_EXEC_TYPE === "sandwich") {
      ...
      const dynamicAttackerAmount =
        volatilityVol > 0 ? volatilityVol.toString() : "5.0";
      const attackerAmountIn = hre.ethers.parseUnits(
        dynamicAttackerAmount,
        WETH_DECIMALS,
      );
    ...
    try {
      ...
      if (DEMO_EXEC_TYPE === "sandwich") {
        ...
        result.victim_output_loss =
          lossNum > 0 ? lossNum.toFixed(6) : "0.000000";
        result.financial_loss_percentage =
          lossPct > 0 ? `${lossPct.toFixed(2)}%` : "0.00%";
      }

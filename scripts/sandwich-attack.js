...
async function main() {
  ...
  try {
    ...
    const { weth: victimWeth, usdc: victimUsdc } = await getTokens(
      WETH_ADDRESS,
      USDC_ADDRESS,
      victimSigner,
    );
    const { weth: attackerWeth, usdc: attackerUsdc } = await getTokens(
      WETH_ADDRESS,
      USDC_ADDRESS,
      attackerSigner,
    );
    ...
    const victimAmountIn = hre.ethers.parseUnits(
      victimAmountInEth,
      WETH_DECIMALS,
    );
    ...
    const attackerAmountIn = hre.ethers.parseUnits(
      attackerAmountInEth,
      WETH_DECIMALS,
    );
    ...
    const attackerWethContract = new hre.ethers.Contract(
      WETH_ADDRESS,
      WETH_ABI_WRAP,
      attackerSigner,
    );
    ...
    const baselineExpectedOutput = hre.ethers.formatUnits(
      baselineQuoteWei,
      USDC_DECIMALS,
    );
    console.log(
      `Baseline expected output for victim: ${baselineExpectedOutput} USDC`,
    );

    const victimUsdcBefore = await victimUsdc.balanceOf(victimSigner.address);
    const attackerWethBefore = await attackerWeth.balanceOf(
      attackerSigner.address,
    );
    ...
    const attackerQuoteWei = await getQuote(
      attackerRouter,
      attackerAmountIn,
      path,
    );
    ...
    const attackerUsdcBalance = await attackerUsdc.balanceOf(
      attackerSigner.address,
    );
    const backrunQuoteWei = await getQuote(
      attackerRouter,
      attackerUsdcBalance,
      reversePath,
    );
    ...
    const attackedActualOutput = hre.ethers.formatUnits(
      victimActualOutputWei,
      USDC_DECIMALS,
    );
    const attackerWethAfter = await attackerWeth.balanceOf(
      attackerSigner.address,
    );
    ...
    console.log(
      `Victim Output Loss: ${outputLossNum.toFixed(6)} USDC (${lossPercentage.toFixed(2)}%)`,
    );
    ...
    const attackerWethDelta = hre.ethers.formatUnits(
      attackerWethDeltaWei,
      WETH_DECIMALS,
    );
    console.log(`Attacker WETH Profit: ${attackerWethDelta} WETH`);

    // Validation Checks
    if (victimActualOutputWei <= 0n)
      throw new Error("Validation Failed: Actual output is 0.");
    if (actualOutputNum >= baselineOutputNum) {
      throw new Error(
        `Validation Failed: Sandwich attack failed! Victim obtained ${actualOutputNum} >= ${baselineOutputNum} baseline. Adjust attacker capital.`,
      );
    }
    ...
    fs.writeFileSync(
      path.join(resultsDir, "sandwich_attack_error.json"),

...
async function runBaseline() {
  console.log(
    `[Exploratory Test] Baseline Swap | ${CUSTOM_AMOUNT} WETH | ${CUSTOM_SLIPPAGE}% slippage`,
  );
...
async function runSandwich() {
  console.log(
    `[Exploratory Test] Sandwich Attack | Victim: ${CUSTOM_AMOUNT} WETH | ${CUSTOM_SLIPPAGE}% slippage`,
  );
  ...
  const baselineQuoteWei = await getQuote(
    victimRouter,
    victimAmountIn,
    swapPath,
  );
  const baselineExpected = hre.ethers.formatUnits(
    baselineQuoteWei,
    USDC_DECIMALS,
  );
...
async function runProtected() {
  console.log(
    `[Exploratory Test] PADGF Protected | ${CUSTOM_AMOUNT} WETH | ${CUSTOM_SLIPPAGE}% slippage`,
  );
  const signer = await setupProviderAndSigner(IMPERSONATED_ACCOUNT);
  const { weth, usdc } = await getTokens(WETH_ADDRESS, USDC_ADDRESS, signer);
  const router = await getRouter(ROUTER_ADDRESS, signer);
  ...
  if (frontrunVolume > 0) {
    ...
    console.log(
      `[Simulation] Inducing mempool state change: ${volatilityVol} WETH traded ahead...`,
    );
    ...
    try {
      ...
      console.log(
        "[Simulation] Volatility induction hit pool limits, proceeding with max available stress.",
      );
    }
  }
  // --------------------------------------------
...
async function main() {
  if (CUSTOM_SCENARIO === "baseline") await runBaseline();
    ...
    console.error("Unknown scenario:", CUSTOM_SCENARIO);
    process.exit(1);
  }
}

...
async function main() {
    ...
    try {
        ...
        const decisionResult = makeDecision(riskMetrics.normalized_risk_score, decisionThresholds);
        ...
        if (decisionResult.execution_allowed) {
            ...
            const receipt = await executeSwap(
                router, weth, amountIn, simulatedQuoteWei, path, signer, slippageTolerance
            );
            ...
            console.log(`Final Output: ${hre.ethers.formatUnits(actualOutputWei, USDC_DECIMALS)} USDC`);
        } else {
            console.log("Execution bypassed by PADGF.");
        }

        // 6. Save results
        const finalOutputStr = (actualOutputWei > 0n) ? hre.ethers.formatUnits(actualOutputWei, USDC_DECIMALS) : simulatedOutput;
